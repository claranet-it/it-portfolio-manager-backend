...
export class ReportService {
    ...
    async getProductivityReport(
        ...
        if (params.customer || params.project || params.task || params.name) {
            return this.getOnlyUsersWithWorkedHours(companyTasks, projectTypes, totalWorkingDaysInPeriod);
        }
        ...
        return await Promise.all(
            allUsers.map(async (user) => {
                ...
                const {
                    ...
                    total,
                    roundedTotal
                } = this.calculatePercentage(userTasks, projectTypes, totalWorkingDaysInPeriod);
    ...
    private calculatePercentage(userTasks: TimeEntryRowType[], projectTypes: {
        project: string;
        projectType: string
    ...
    private async getOnlyUsersWithWorkedHours(companyTasks: TimeEntryRowType[], projectTypes: {project: string; projectType: string }[], totalWorkingDaysInPeriod: number) {
        const userTasks: TimeEntryRowType[] = this.groupTasksByUser()
        return await Promise.all(
            companyTasks.map(async (task) => {
                 const userInfo =
                    await this.userProfileRepository.getCompleteUserProfile(task.user)
                const {
                    workedHours,
                    billableProductivityPercentage,
                    nonBillableProductivityPercentage,
