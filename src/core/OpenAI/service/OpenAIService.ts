...
export class OpenAIService {
  ...
  private async getRequestedSkills(
    allSkills: string[],
    question: string,
  ): Promise<{ skills: string[] }> {
    const prompt = process.env
      .FIND_SKILLS_OPENAI_PROMPT!.replace('[[QUESTION]]', question)
      .replace('[[SKILL_LIST]]', JSON.stringify(skillsList))
    return JSON.parse(await this.getResponse(prompt))
  }

  private async getEffortPeriod(
    question: string,
  ): Promise<{ start: string; month_number: number }> {
    const prompt = process.env
      .FIND_PERIOD_OPENAI_PROMPT!.replace('[[QUESTION]]', question)
      .replace(
        '[[CURRENT_YEAR]]',
  ...
  async answerQuestionWithSkillsAndEffort(
    question: string,
    company: string,
    ...
    const peopleWithSkills = this.filterPeopleWithSkills(
      skills,
      filterdSills.skills,
    )
    const effortPeriod = await this.getEffortPeriod(question)
    const effort = (
      await this.effortService.getEffortPeriod(
        peopleWithSkills.map((p) => p.uid),
        effortPeriod.start,
        effortPeriod.month_number,
      )
    ).map((e) => {
      return {
        uid: e.uid,
    ...
    const prompt = process.env
      .FIND_TEAM_OPENAI_PROMPT!.replace(
        '[[SKILL]]',
        JSON.stringify(peopleWithSkills),
      )
  ...
  private filterPeopleWithSkills(
    peopleWithSkills: SkillMatrixList,
    skills: string[],
  ) {
