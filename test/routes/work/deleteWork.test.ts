...
test('should return 500 deleting non existing item in curriculum work array', async (t) => {
  const response = await deleteWorkItem(app, getToken(app, FAKE_EMAIL), 'not_valid_id')
  t.equal(response.statusCode, 500)
})

test('should delete work item in curriculum', async (t) => {
  const response = await deleteWorkItem(app, getToken(app, FAKE_EMAIL), 'delete')
  t.equal(response.statusCode, 204)

  const getResponse = await getCurriculum(app, getToken(app, FAKE_EMAIL))
  const getResponseData = getResponse.json()
  t.equal(getResponseData.work.length, 1)
  t.equal(getResponseData.email, FAKE_CURRICULUM_DATA_WORK_DELETED.email)
  t.equal(getResponseData.name, FAKE_CURRICULUM_DATA_WORK_DELETED.name)
})
